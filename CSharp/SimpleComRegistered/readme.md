...
There is an nuget package called Excel DNA that make it simple to call .NET from Excel, however, it is compatible only up to .NET 6. At the time of this project, this version is out of support. .NET versions that are out of support can be restricted from being used in professional development environments.
...
There are also inconveniences to using Excel DNA with .NET 6. Even if it was allowed, end users must download the .NET 6 runtime to use it. It is also about half the speed of .NET 9.
